...
- **Routing as data** – A function is a tool name and a destination: a node, or a branch table keyed on a field of the tool's result. A function that only moves the conversation is written in the config alone, as a transition-only entry with a description, and needs no Python. On the canvas a function that leads somewhere is an edge labeled with its name, a branch is a decision diamond with one labeled edge per case, and a card keeps the functions that stay on the node.
...
- The canvas owns structure and the inspector owns content. Flows read top to bottom: each card has one entry at the top and one exit at the bottom, a function that leads somewhere is an edge labeled with its name, and a branch is a diamond with the field it keys on inside and one edge per case, labeled with the case value, leaving by the side that faces its target. A case that leads back to its own node loops around the card, as a function that leads to its own node does; a function that stays on the node is a row on the card, with a warning when its destination no longer exists. A card shows two lines of the node's first task message under its name, an end node with nothing but the end is a small pill. Global functions, offered at every node, are one dashed card, "Every node", with a row per function: a tool that stays where the conversation is, or one that leads somewhere, with where. It has no edges, since it could leave from any node. Click a row to open the function in the Flow panel.
- Hover a node and press its "+" to add a function: leading to a next node, an end node, or a branch on the tool's result, or staying on the node. "Add case" on a branch in the sidebar adds a case leading to a new node. Every node is reachable by construction. Double-click a node name, or a branch's field in its diamond, to rename it in place; function names and case values rename in the sidebar. Click an edge's label or a diamond to select its function or case; hover a row for its "×". Delete removes what is selected: an edge's route or a node.
...
- Routing lives on functions as `transition_to`: a node name, or a branch table with `field`, `cases`, and an optional `default`. Drag from anywhere along a card's bottom edge to a node to add a function leading there, or from a diamond's tip to add a case; change a destination in the sidebar.
...
- Canvas positions are not part of the document. A freshly opened file is laid out top to bottom, with edges routed around the nodes between their ranks and a slot for every label; positions are then kept in `localStorage`, keyed by flow name. A node dragged by hand carries its edges with it, and **Layout** lays everything out again.
